...
This repository is an installable Codex Skill and a standalone local tool for turning arbitrary SKU fitment into an estimated effective vehicle population, a Core/Discovery listing matrix, required multi-model Mixed titles, and a PLP campaign matrix. It ships without product or SKU data.
...
Every SKU with at least two distinct compatible models receives one additional Mixed listing. Its title follows descending market rank and its Compatibility covers the full SKU fitment. Core and Discovery remain the mutually exclusive primary pools; the supplemental Mixed listing is excluded from PLP generation to prevent keyword competition.
